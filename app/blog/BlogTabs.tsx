  ...
  return (
    <>
      ...
      <div className="px-5 pt-5 pb-2">
        <p className="text-[10px] font-bold tracking-[0.18em] text-stone-400 mb-2">FEATURED</p>
        <Link
        ...
        >
          ...
          <div style={{ position: 'relative' }}>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-[10px] bg-shu text-white px-2 py-0.5 rounded-full font-bold tracking-wider">PICK UP</span>
              <span className="text-[11px] text-shu font-bold">{fmt(featured.date)}</span>
            </div>
            ...
            <p className="text-xs text-stone-300 leading-relaxed line-clamp-2 mb-3">{featured.summary}</p>
            <span className="inline-flex items-center gap-1 text-xs text-shu font-bold">
              記事を読む
      ...
      {/* Category tabs */}
      <div className="flex border-b border-stone-200 px-4 sticky top-0 bg-white z-10 select-none mt-4">
        {CATEGORIES.map(cat => (
          <button
            key={cat}
      ...
      <div className="px-5 pt-4 pb-2 flex items-baseline justify-between">
        <h2 className="text-[11px] font-bold tracking-[0.18em] text-stone-400">
      ...
      <div className="px-5 pb-6">
        {filtered.map((post, i) => (
          ...
          >
            <span className="text-[13px] font-black text-stone-300 mt-0.5 flex-shrink-0" style={{ minWidth: 28, fontVariantNumeric: 'tabular-nums' }}>
              {String(i + 1).padStart(2, '0')}
            </span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-[9px] font-bold text-shu bg-shu-bg px-1.5 py-0.5 rounded-full tracking-wide">{post.category}</span>
                <p className="text-[10px] text-stone-400 font-semibold" style={{ fontVariantNumeric: 'tabular-nums' }}>{fmt(post.date)}</p>
              </div>
              <p className="text-[13.5px] font-bold text-stone-800 leading-snug mb-0.5 group-hover:text-shu transition-colors" style={{ textWrap: 'pretty' as React.CSSProperties['textWrap'] }}>{post.title}</p>
